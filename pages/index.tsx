              ...
              {loading ? (<></>) : (
                <Flex justifyContent="flex-end" >
                  <Box background={"teal.100"} borderRadius={"5px"} minWidth={"50px"} p={2} >
                    <VStack >
                      <Text color= '#02395d' font-family='acier-bat-solid' fontWeight={"semibold"}>Available NFTs: {Number(candyMachine?.data.itemsAvailable) - Number(candyMachine?.itemsRedeemed)}/{Number(candyMachine?.data.itemsAvailable)}</Text>
                    </VStack>
                  </Box>
                </Flex>
              )}
